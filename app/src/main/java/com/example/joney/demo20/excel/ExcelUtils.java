package com.example.joney.demo20.excel;

/**
 * Created by joney on 2018/4/13.
 */
                ...
                for (int j = 0; j < objList.size(); j++) {
                    ...
                    for (int i = 0; i < list.size(); i++) {
                        ...
                        if (list.get(i).length() <= 5){
                            ...
                            sheet.setColumnView(i,list.get(i).length()+10); //设置列宽
                        }
                    }
                }

                writebook.write();
                Toast.makeText(c, "成功导出到手机文件夹", Toast.LENGTH_SHORT).show();
            } catch (Exception e) {
                e.printStackTrace();
            } finally {
                if (writebook != null) {
